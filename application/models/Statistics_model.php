...
class Statistics_model extends CI_Model {
    ...
    function Statistics_1_avg($Date_1,$Date_2){
      $this->db->select('area_town.Town_name, area_town.lat, area_town.lng,  ROUND(COUNT(*)/town_info.人口數*100000,4) AS amount');
      $this->db->from('files_info_table');
      $this->db->join('area_town', 'area_town.Town_code = files_info_table.town','left');
      $this->db->join('town_info', 'town_info.區名 = area_town.Town_name','left');
      $this->db->where("`files_info_table`.`建案日期` BETWEEN '{$Date_1}' and '{$Date_2}'");
      $this->db->where("案件刪除", 0);
      $this->db->group_by('town');
      $this->db->order_by('amount');
  ...
  "SELECT 
    ...
    COUNT(case when `扶助級別` like '%不符%'      then `files_info_table`.`扶助級別` end) as `不符`
   from
      `miliboy_table`
