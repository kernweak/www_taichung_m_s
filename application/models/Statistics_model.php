...
class Statistics_model extends CI_Model {
  ...
  //   	ini_set('xdebug.var_display_max_depth', 5);
    ...
    function Statistics_3($Date_1,$Date_2){
    	$Qstring = "SELECT `area_town`.`Town_name` AS `區別`, COUNT((case 
            when `扶助級別` like '%甲級%'      then `files_info_table`.`扶助級別`
       end)) as `甲級`,
       COUNT(case 
            when `扶助級別` like '%乙級%'      then `files_info_table`.`扶助級別`
       end) as `乙級`,
       COUNT(case 
            when `扶助級別` like '%丙級%'      then `files_info_table`.`扶助級別`
       end) as `丙級`
   from
      `miliboy_table`
   ...
   where
    ...
    function Statistics_4($Date_1,$Date_2){
    	$Qstring = "SELECT case when `files_info_table`.`扶助級別` like '%甲級%' then '甲級' when `files_info_table`.`扶助級別` like '%乙級%' then '乙級' when `files_info_table`.`扶助級別` like '%丙級%' then '丙級' end as GroupName, count(*) as PostCount from `miliboy_table` left join`files_info_table` on `files_info_table`.`案件流水號` = `miliboy_table`.`最新案件流水號` left join `area_town` on `area_town`.`Town_code` =`files_info_table`.`town` left join`family_members` on `family_members`.`案件流水號` = `files_info_table`.`案件流水號` where (`family_members`.`特殊身分類別` != 1) AND (`是否扶助` = 1) AND ( `files_info_table`.`扶助級別` like '%甲級%' OR `files_info_table`.`扶助級別` like '%乙級%' OR `files_info_table`.`扶助級別` like '%丙級%' ) GROUP BY GroupName";
    	$query = $this->db->query($Qstring);
		$result = $query->result_array();
		return $result;
    }
